...
exports.CodeGraphAPI = void 0;
exports.analyzeCodebase = analyzeCodebase;
exports.findRelatedCode = findRelatedCode;
...
class CodeGraphAPI {
    ...
    constructor(repoPath, dbPath, semantic = true) {
        ...
        if (!(0, fs_1.existsSync)(this.dbPath)) {
            ...
            throw new Error(`Database not found at ${this.dbPath}. Run '${scanCmd}' first.`);
        }
     ...
     */
    getEdges(source, target, edgeType) {
     ...
     */
    getImpactRadius(symbol, maxDepth = 3) {
        const impacted = new Set();
        const toProcess = [[symbol, 0]];
    ...
    findCycles() {
        ...
        const allSymbols = stmt.all().map((row) => row.fqn);
        const dfs = (node) => {
            visited.add(node);
            recStack.push(node);
...
exports.CodeGraphAPI = CodeGraphAPI;
 ...
 */
function findRelatedCode(repoPath, symbol) {
    const api = new CodeGraphAPI(repoPath);
    const result = {
        symbol,
...
//# sourceMappingURL=code-graph-api.js.map